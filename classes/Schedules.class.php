...
class Schedules extends Dbh
{
  ...
  protected function setSchedule($timeFrom, $timeTo, $profID, $subjID, $rmID, $sectID, $schoolYearID)
  {
    $sql = 'INSERT INTO schedules (sched_from,sched_to,prof_id,subj_id,room_id,sect_id,school_year_id) VALUES(?,?,?,?,?,?,?)';
    try {
      $stmt = $this->connect()->prepare($sql);
      $stmt->execute([$timeFrom, $timeTo, $profID, $subjID, $rmID, $sectID, $schoolYearID]);
    } catch (PDOException $e) {
      trigger_error('Error: ' . $e);
    }
  }
  ...
  protected function getTimeSlotValue($type, $id, $schoolYearID)
  {
    $sql = "SELECT sc.sched_id,sched_from,sched_to,sched_day,last_name,rm_name,sect_name,subj_desc,school_year_id FROM schedules sc
    ...
    LEFT JOIN subjects su ON sc.subj_id = su.subj_id WHERE sc.{$type}_id = ? AND school_year_id = ?";
    try {
      $stmt = $this->connect()->prepare($sql);
  ...
  protected function getScheduleByTypeID($type, $id, $schoolYearID)
  {
  ...
  protected function getScheduleByTimeAndDay($timeFrom, $timeTo, $days, $schedID, $schoolYearID)
  {
    $newDays = '';
